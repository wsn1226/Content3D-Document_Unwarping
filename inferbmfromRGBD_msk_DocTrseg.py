...
def unwarp_fast_torchblur(img, bm):
    img=convertimg(img)
    h,w=img.shape[2],img.shape[3]
    #h,w=1080,1920
    #print(w,h)
    ...
    #bm0=smooth2D(bm[:,:1,:,:],weights,pad='replicate')
    #bm1=smooth2D(bm[:,1:,:,:],weights,pad='replicate')
    bm=T.GaussianBlur((3,3),(0.8,1.2))(bm)
    #print(bm0.shape)
    #bm0=F.interpolate(bm0,(h,w),mode='bilinear',align_corners=True)
    #bm1=F.interpolate(bm1,(h,w),mode='bilinear',align_corners=True)
    bm=F.interpolate(bm,(h,w),mode='bilinear',align_corners=True)
...
def test(args,img_path,fname,depth_constraint=False):
    ...
    reload_segmodel(boundary_model, args.boundary_model_path)
    boundary_model.eval()
    ...
    with torch.no_grad():
        ...
        pred_depth= activation(depth_model(extracted_img)[0])
        ...
        #pred_depth=final_msk*((pred_depth+0.675)/(0.675+maxd))
        time2=time.time()
        #print(time2-time1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_path', nargs='?', type=str, default='/disk2/sinan/mymodelresult/6blocks_joint_doctrseg_test_6blocks_158_0-1',
...
#CUDA_VISIBLE_DEVICES=0 python inferbmfromRGBD_msk_DocTrseg.py --boundary_model_path /home/sinan/DewarpNet-master/model_pretrained_doctr/seg.pth --depth_model_path /home/sinan/DewarpNet-master/checkpoints-jointmskdepth_6blocks-u2depth_full/u2net_depth_158_0.03670893407193316_joint_masked_depth_best_model.pkl --bm_model_path /home/sinan/DewarpNet-master/checkpoints-jointmskdepth_6blocks-bm_full/mobilevit_sandfcn_skipRGBD_6blocks_158_0.011349096665826609_joint_masked_depth_best_model.pkl --show
""" 
