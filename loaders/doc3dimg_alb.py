...
class doc3dbm_mobilevitfcn_direct_loader_norecon(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        albedo_path = pjoin(self.root, 'albedo/alb' , im_name + '.png')
        ...
        albedo=cv2.imread(albedo_path)[:,:,::-1]/255.0
        ...
        if self.is_transform:
            im, lbl = self.transform(alb,albedo, bm)
    ...
    def tight_crop(self, alb,albedo):
        ...
        return alb,albedo,t,b,l,r


    def transform(self, alb, albedo, bm):
        #timestarttransform=time.time()
        alb,albedo, t,b,l,r=self.tight_crop(alb,albedo)               #t,b,l,r = is pixels cropped on top, bottom, left, right
        ...
        albedo = m.imresize(albedo, self.img_size)
        albedo = albedo.astype(float)/255.0
        albedo = albedo.transpose(2, 0, 1) # NHWC -> NCHW
        ...
        albedo = torch.from_numpy(albedo).to(torch.float32)
        img = torch.cat([alb,albedo],dim=0)
