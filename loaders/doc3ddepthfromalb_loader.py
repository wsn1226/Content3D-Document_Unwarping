...
class doc3ddepthfromalb_loader(data.Dataset):
    """
    Data loader.
    """
    ...
    def tight_crop_d(self, im, dm):
        ...
        miny = min(y)
        maxy = max(y)
        maxd=np.max(dm[msk.nonzero()])
        mind=np.min(dm[msk.nonzero()])
        dm[msk.nonzero()]=(dm[msk.nonzero()]-mind)/(maxd-mind)+1
    ...
    def transform(self, alb, lbl):
        ...
        lbl = np.expand_dims(lbl,axis=0)-1  # HW -> CHW
...
#with torch.autograd.profiler.profile(enabled=True) as prof:
    dst = doc3ddepthfromalb_loader(root='/disk2/sinan/doc3d/', split='trainmini', is_transform=True, img_size=(256,256))
    ...
    for i, data in enumerate(trainloader):
        ...
        for i in range(0,imgs.shape[0]):
            cv2.imwrite('testunwarp'+str(i)+'.png',imgs[i].permute(1,2,0).cpu().numpy()*255)
