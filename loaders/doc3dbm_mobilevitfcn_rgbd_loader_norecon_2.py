...
class doc3dbm_mobilevitfcn_rgbd_loader_norecon_2(data.Dataset):
    """
    Data loader.
    """
    ...
    def __getitem__(self, index):
        ...
        d_path = pjoin(self.root, 'depth_pred' , im_name + '.npy')
        #print(d_path)
    ...
    def transform(self, alb,dmap, bm):
        ...
        bm[:,:,1]=bm[:,:,1]-t*7/4
        bm[:,:,0]=bm[:,:,0]-l*7/4
        bm=bm/np.array([448.0-l*7/4-r*7/4, 448.0-t*7/4-b*7/4])
        bm=(bm-0.5)*2
...
#with torch.autograd.profiler.profile(enabled=True) as prof:
    dst = doc3dbm_mobilevitfcn_rgbd_loader_norecon_2(root='/disk2/sinan/doc3d/', split='trainmini', is_transform=True, img_size=(128,128))
    ...
    for i, data in enumerate(trainloader):
        ...
        #for i in range(12):
            #print(torch.max(imgs[i][:,:,3:]),torch.min(imgs[i][:,:,3:]))
        for i in range(0,labels.shape[0]):
            #print(labels[i])
            #print()
            cv2.imwrite('testunwarp'+str(i)+'.png',unwarpedimg[i].permute(1,2,0).numpy()*255)
            #cv2.imwrite('testdepth'+str(i)+'.png',imgs[i][3:,:,:].permute(1,2,0).numpy()*255)
            #cv2.imwrite('test'+str(i)+'.png',imgs[i][:3,:,:].permute(1,2,0).numpy()*255)
