...
class doc3dbm_rgb_wc(data.Dataset):
    """
    Data loader.
    """
    ...
    def __getitem__(self, index):
        ...
        if self.is_transform:
            rgbd, lbl = self.transform(alb,wc,bm)
    ...
    def tight_crop(self,im, wc):
        ...
        l=minx-s+cx1
        r=size[1]-maxx-s+cx2
        return im,wc,t,b,l,r


    def transform(self, alb,wc, bm):
        #timestarttransform=time.time()
        alb,wc,t,b,l,r=self.tight_crop(alb,wc)               #t,b,l,r = is pixels cropped on top, bottom, left, right
        #print(alb.shape,t,b,l,r)
        #cv2.imwrite('testrecon.png',recon)
        #cv2.imwrite('testimg.png',alb.cpu().numpy())
        #timefinishcrop=time.time()
        #print((wc<0).astype(np.uint8).nonzero())
...
#with torch.autograd.profiler.profile(enabled=True) as prof:
    dst = doc3dbm_rgb_wc(root='/disk2/sinan/doc3d/', split='trainmini', is_transform=True, img_size=(128,128))
