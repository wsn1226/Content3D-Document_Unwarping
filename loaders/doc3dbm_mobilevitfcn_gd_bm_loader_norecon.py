...
class doc3dbm_mobilevitfcn_gd_bm_loader_norecon(data.Dataset):
    """
    Data loader.
    """
    ...
    def tight_crop(self, alb, dmap):
        ...
        msk_3d=np.repeat(np.expand_dims(msk_dmap,-1),3,axis=-1)
        if 'train' in self.split:
            alb=self.color_jitter(alb,0.2,0.2,0.6,0.6)
            #recon=self.color_jitter(recon,0.2,0.2,0.6,0.6)
            alb=msk_3d*alb
    ...
    def transform(self, alb,dmap, bm):
        ...
        alb = cv2.cvtColor(alb,cv2.COLOR_RGB2GRAY)
        alb = np.expand_dims(alb.astype(float)/255.0,0)
...
#with torch.autograd.profiler.profile(enabled=True) as prof:
    dst = doc3dbm_mobilevitfcn_gd_bm_loader_norecon(root='/disk2/sinan/doc3d/', split='trainmini', is_transform=True, img_size=(128,128))
    ...
    for i, data in enumerate(trainloader):
        ...
        unwarpedimg=F.grid_sample(imgs[:,:1,:,:],labels[:,:,:,:])
        ...
        for i in range(0,labels.shape[0]):
            ...
            cv2.imwrite('testimg'+str(i)+'.png',imgs[i,0,:,:].numpy()*255)
