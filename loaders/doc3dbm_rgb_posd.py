...
class doc3dbm_rgb_posd(data.Dataset):
    """
    Data loader.
    """
    ...
    def transform(self, alb,dmap, bm):
        ...
        ensuremsk=(dmap>=0).to(torch.float).repeat(3,1,1)
        posd = torch.cat([originalgrid.permute(2,0,1),dmap],dim=0)
        rgb_posd=torch.cat([alb,posd*ensuremsk],dim=0)
...
#with torch.autograd.profiler.profile(enabled=True) as prof:
    dst = doc3dbm_rgb_posd(root='/disk2/sinan/doc3d/', split='valmini', is_transform=True, img_size=(128,128))
