...
def train(args):
    ...
    # Setup Dataloader
    data_loader = get_loader('doc3d_joint_rgbp_flow_withrecon')
    ...
    model_joint=get_model(name='u2net_joint_rgbp_reconin',boundary_model=model_boundary, prob_model=model_prob,flow_model=model_flow)
    ...
    experiment_name='joint_rgbp_reconin'
    ...
    for epoch in range(epoch_start,args.n_epoch):
        ...
        for i, (images, recon, pmap, flow) in enumerate(trainloader):
            ...
            labels_msk=1-((recon[:,:,0]==0)&(recon[:,:,1]==0)&(recon[:,:,2]==0)).astype(np.uint8)
            #print(msk_index.shape)

            b1,b2,b3,b4,b5,b6,b7,pred_prob,pred_flow,pred_flowfromrecon = model_joint(images,recon,(args.flow_img_rows,args.flow_img_cols))
            ...
            loss=loss_boundary+loss_prob+loss_flow+loss_flowfromrecon+loss_ad #+0.05*rloss+0.05*imgrloss
            ...
            if (i+1) % 50 == 0:
                ...
                print("Epoch[%d/%d] Batch [%d/%d] Boundary Loss: %.4f Depth Loss: %.4f BM Loss: %.4f BM From Recon: %.4f AD: %.4f" % (epoch+1,args.n_epoch,i+1, len(trainloader),avg_loss_boundary/50.0, avg_loss_prob/50.0, avg_loss_flow/50.0,avg_loss_flowfromrecon/50.0, avg_loss_ad/50.0))
                avg_loss_prob=0.0
                avg_loss_flow=0.0
                avg_loss_boundary=0.0
                avg_loss_flowfromrecon=0.0
            ...
            if args.tboard and  (i+1) % 20 == 0:
                ...
                writer.add_scalar('Depth: L1 Loss/train', avgl1loss_prob/(i+1), global_step)
                writer.add_scalar('BM: L1 Loss/train', avgl1loss_flow/(i+1), global_step)
                writer.add_scalar('BM From Recon: L1 Loss/train', avgl1loss_flowfromrecon/(i+1), global_step)
            del b1,b2,b3,b4,b5,b6,b7,d0, pred_prob, pmap, pred_flow,predflow_nhwc,pred_msk,pred_flowfromrecon,loss,loss_flow,loss_boundary,loss_prob,loss2_boundary,loss2_prob
            if (i+1) % 50==0:
                torch.cuda.empty_cache()

        avg_loss2_boundary=avg_loss2_boundary/len(trainloader)
        avg_loss2_prob=avg_loss2_prob/len(trainloader)
        ...
        print("Training Boundary loss2:%4f prob loss2:%4f BM L1:%4f Recon:%4f" %(avg_loss2_boundary, avg_loss2_prob, avgl1loss_flow, avgl1loss_flowfromrecon))
        ...
        train_losses_prob=[avg_loss2_prob]
        ...
        for i_val, (images_val, recon, prob_val, flow_val) in tqdm(enumerate(valloader)): #use progress bar
            with torch.no_grad():
                ...
                b1,b2,b3,b4,b5,b6,b7,pred_prob,pred_flow,pred_flowfromrecon= model_joint(images_val,recon,(args.flow_img_rows,args.flow_img_cols))
                pred_msk=(b1>0.5).to(torch.float32)
                ...
                loss_prob=l1_loss(only_d1,only_d_lbl)
                ...
                del b1,b2,b3,b4,b5,b6,b7,d1,d2,d3,d4,d5,d6,d7,only_d_lbl,only_d1,labels_msk,predflow_nhwc,pred_flow,pred_msk,pred_flowfromrecon,recon
                if (i_val+1) % 50==0:
                    torch.cuda.empty_cache()
        ...
        if args.tboard:
            ...
            writer.add_scalar('Depth: L1 Loss/val', val_loss_prob/len(valloader), epoch+1)
            writer.add_scalar('BM: L1 Loss/val', val_loss_flow/len(valloader), epoch+1)
        ...
        print("Depth val loss at epoch {}:: {}".format(epoch+1,val_loss_prob))
        print("BM val loss at epoch {}:: {}".format(epoch+1,val_loss_flow))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--arch_prob', nargs='?', type=str, default='mobilevit_sandfcn_skip_prob', 
                        help='Architecture to use')
    ...
    parser.add_argument('--resume_boundary', nargs='?', type=str, default=None,    
                        help='Path to previous saved model to restart from')
    parser.add_argument('--logdir_prob', nargs='?', type=str, default='./checkpoints-joint_rgbp_prob/',    
                        help='Path to store the loss logs')
    parser.add_argument('--logdir_flow', nargs='?', type=str, default='./checkpoints-joint_rgbp_flow/',    
                        help='Path to store the loss logs')
    parser.add_argument('--logdir_boundary', nargs='?', type=str, default='./checkpoints-joint_rgbp_boundary/',    
                        help='Path to store the loss logs')
...
#CUDA_VISIBLE_DEVICES=1,0,2 python train_joint_rgbp_flow_reconin.py --data_path /disk2/sinan/doc3d/ --batch_size 30 --tboard --resume_boundary /home/sinan/DewarpNet-master/checkpoints-boundary_fullreconin_originalpmap_0.1bloss/u2net_329_0.06557631423462651_joint_masked_probreconin_best_model.pkl --resume_prob /home/sinan/DewarpNet-master/checkpoints-u2prob_fullreconin_originalpmap_0.1bloss/u2net_prob_329_0.03506685778900178_joint_masked_probreconin_best_model.pkl --resume_flow /home/sinan/DewarpNet-master/checkpoints-flow_fullreconin_originalpmap_0.1bloss/mobilevit_sandfcn_skipRGBD_329_0.010997372938732722_joint_masked_probreconin_best_model.pkl