...
def train(args):
    ...
    model_joint=get_model(name='u2netdepth_joint_mobilevit_full',boundary_model=model_boundary, depth_model=model_depth,bm_model=model_bm)
    optimizer_joint= torch.optim.Adam(model_joint.parameters(),lr=args.l_rate, weight_decay=5e-4, amsgrad=True)
    sched_joint=torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_joint, mode='min', factor=0.5, patience=3, verbose=True)
    ...
    for epoch in range(epoch_start,args.n_epoch):
        ...
        for i, (images,recon, dmap, bm) in enumerate(trainloader):
            ...
            loss=loss_boundary+loss_depth+loss_bm+0.1*rloss
            ...
            if (i+1) % 50 == 0:
                print("Epoch[%d/%d] Batch [%d/%d] Boundary Loss: %.4f Depth Loss: %.4f BM Loss: %.4f Recon Loss: %.4f" % (epoch+1,args.n_epoch,i+1, len(trainloader),avg_loss_boundary/50.0, avg_loss_depth/50.0, avg_loss_bm/50.0,avgrloss/50.0))
                avg_loss_depth=0.0
                avg_loss_bm=0.0
                avg_loss_boundary=0.0
                avgrloss=0.0
            ...
            del b1,b2,b3,b4,b5,b6,b7,d0, d1, d2, d3, d4, d5, d6,only_d0, only_d1, only_d2, only_d3, only_d4, only_d5, only_d6,only_d_lbl, pred_bm,predbm_nhwc,pred_msk,loss,loss_bm,loss_boundary,loss_depth,loss2_boundary,loss2_depth
            if (i+1) % 50==0:
        ...
        print("Training Boundary loss2:%4f depth loss2:%4f BM L1:%4f" %(avg_loss2_boundary, avg_loss2_depth, avgl1loss_bm))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', nargs='?', type=int, default=200, 
                        help='Batch Size')
    parser.add_argument('--l_rate', nargs='?', type=float, default=1e-05, 
                        help='Learning Rate')
    ...
    parser.add_argument('--resume_boundary', nargs='?', type=str, default=None,    
                        help='Path to previous saved model to restart from')
    parser.add_argument('--logdir_depth', nargs='?', type=str, default='./checkpoints-jointmskdepth-u2depth_full_withrecon/',    
                        help='Path to store the loss logs')
    parser.add_argument('--logdir_bm', nargs='?', type=str, default='./checkpoints-jointmskdepth-bm_full_withrecon/',    
                        help='Path to store the loss logs')
    parser.add_argument('--logdir_boundary', nargs='?', type=str, default='./checkpoints-jointmskdepth-boundary_full_withrecon/',    
                        help='Path to store the loss logs')
...
#CUDA_VISIBLE_DEVICES=2,0,1 python trainjoint_masked_depth_full_withrecon.py --data_path /disk2/sinan/doc3d/ --batch_size 33 --tboard --resume_boundary /home/sinan/DewarpNet-master/checkpoints-jointmskdepth-boundary_full_withrecon/u2net_274_0.046370445649222575_joint_masked_depth_best_model.pkl --resume_depth /home/sinan/DewarpNet-master/checkpoints-jointmskdepth-u2depth_full_withrecon/u2net_depth_274_0.035387783990588915_joint_masked_depth_best_model.pkl --resume_bm /home/sinan/DewarpNet-master/checkpoints-jointmskdepth-bm_full_withrecon/mobilevit_sandfcn_skipRGBD_274_0.011687130860910557_joint_masked_depth_best_model.pkl