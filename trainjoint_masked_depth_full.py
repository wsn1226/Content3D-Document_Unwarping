...
def train(args):
    ...
    for epoch in range(epoch_start,args.n_epoch):
        ...
        for i, (images, dmap, bm) in enumerate(trainloader):
            ...
            loss=loss_boundary+loss_depth+loss_bm
            ...
            if (i+1) % 50 == 0:
                print("Epoch[%d/%d] Batch [%d/%d] Boundary Loss: %.4f Depth Loss: %.4f BM Loss: %.4f" % (epoch+1,args.n_epoch,i+1, len(trainloader),avg_loss_boundary/50.0, avg_loss_depth/50.0, avg_loss_bm/50.0 ))
                avg_loss_depth=0.0
                avg_loss_bm=0.0
                avg_loss_boundary=0.0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--arch_depth', nargs='?', type=str, default='u2net_depth', 
                        help='Architecture to use')
    parser.add_argument('--arch_bm', nargs='?', type=str, default='DocTr', 
                        help='Architecture to use')
    ...
    parser.add_argument('--resume_boundary', nargs='?', type=str, default=None,    
                        help='Path to previous saved model to restart from')
    parser.add_argument('--logdir_depth', nargs='?', type=str, default='./checkpoints-jointDocTr_depth/',    
                        help='Path to store the loss logs')
    parser.add_argument('--logdir_bm', nargs='?', type=str, default='./checkpoints-jointDocTr_bm/',    
                        help='Path to store the loss logs')
    parser.add_argument('--logdir_boundary', nargs='?', type=str, default='./checkpoints-jointDocTr_boundary/',    
                        help='Path to store the loss logs')
...
#CUDA_VISIBLE_DEVICES=1,0,2 python trainjoint_masked_depth_full.py --data_path /disk2/sinan/doc3d/ --tboard --resume_boundary /home/sinan/DewarpNet-master/checkpoints-u2net-nopre/u2net_132_0.010844790506031037_0.04020660579093357_boundary_best_model.pkl --resume_depth /home/sinan/DewarpNet-master/checkpoints-u2depth/u2net_depth_254_0.03333024032177337_0.05067401671253923_depth_best_model.pkl --resume_bm /home/sinan/DewarpNet-master/checkpoints-rgbd_DocTr_norm/DocTr_170_0.0001158251697493792_8.551328645437366e-05_DocTr_best_model.pkl