...
def test(args,img_path,fname,wc_constraint=False):
    originalstart=time.time()
    #wc_model_file_name = os.path.split(args.wc_model_path)[1]
    #wc_model_name = wc_model_file_name[:wc_model_file_name.find('_',2)]
    boundary_model_name='u2net'
    #wc_model_name='mobilevit_xxsandfcn_skip_wcfromalb'
    wc_model_name='u2net_nm'
    ...
    bm_model_name='mobilevit_sandfcn_skip_rgb_wc'
    ...
    h,w,_=imgorg.shape
    imgorgbgr=cv2.resize(imgorg, wc_img_size)
    ...
    imgrgb = cv2.resize(imgorgrgb, wc_img_size)
    #img = img[:, :, ::-1]
    ...
    with torch.no_grad():
        ...
        pred_wc= wc_model(extracted_img)[0]*mskorg
        if args.norm:
            norm=pred_wc.pow(2).sum(dim=1).sqrt()
            msk=norm.nonzero()
            norm=norm.unsqueeze(0)
            pred_wc[msk[:,0],:,msk[:,1],msk[:,2]] = pred_wc[msk[:,0],:,msk[:,1],msk[:,2]]/norm[msk[:,0],:,msk[:,1],msk[:,2]]
        #print(torch.max(pred_wc),torch.min(pred_wc))
        time2=time.time()
        #print(time2-time1)

        bm_input=torch.cat([extracted_img,pred_wc],dim=1)
        bm_input=F.interpolate(bm_input,bm_img_size,mode='bilinear',align_corners=True)
        #print(bm_input.shape)
        cv2.imwrite("testnm.png",bm_input[0,3:,:,:].permute(1,2,0).cpu().numpy()*255)
    ...
    #cv2.imwrite(os.path.join(args.aux_out_path,fname),(mskorg*(pred_wc/maxd)).squeeze(0).permute(1,2,0).cpu().numpy()*255)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_path', nargs='?', type=str, default='/disk2/sinan/mymodelresult/final_rgb_nm_u2net',
    ...
    for fname in os.listdir(args.img_path):
        if '.jpg' in fname or '.JPG' in fname or '.png' in fname:
            ...
            totalnbofimg+=1
            totaltime+=test(args,img_path,fname,wc_constraint=False)
    print(totaltime/totalnbofimg)

#CUDA_VISIBLE_DEVICES=0 python inferbmfromRGBWC_Unet.py --boundary_model_path /home/sinan/DewarpNet-master/checkpoints-u2net-nopre/u2net_132_0.010844790506031037_0.04020660579093357_boundary_best_model.pkl --wc_model_path /home/sinan/DewarpNet-master/checkpoints-nm_U2NET/u2net_nm_119_0.005878953804735574_0.007961844027149506_doc3dnm_u2net_best_model.pkl --bm_model_path /home/sinan/DewarpNet-master/checkpoints-rgb_nm/mobilevit_sandfcn_skip_rgb_wc_113_0.00010615365114063025_0.00014921877347715725_rgb_nm_best_model.pkl --show
""" 
