...
class u2net_joint_rgbp_reconin(nn.Module):
    ...
    def forward(self, images,recon, flow_img_size):
        ...
        rgbd=torch.cat([rgb,prob],dim=1)
        recond=torch.cat([recon,prob],dim=1)
        
        pred_flow=self.flow_model(rgbd)
        pred_flowfromrecon=self.flow_model(recond)

        return b1,b2,b3,b4,b5,b6,b7,pred_prob,pred_flow,pred_flowfromrecon