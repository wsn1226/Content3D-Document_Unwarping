...
class MobileViT_FCN(nn.Module):
    def __init__(self, image_size, dims, channels, num_classes, expansion=4, kernel_size=3, patch_size=(2, 2),input_channels=3,mode=None):
        ...
        self.mvit.append(MobileViTBlock_v2(dims[0], L[0], channels[5], kernel_size, patch_size, int(dims[0]*2)))
        self.mvit.append(MobileViTBlock_v2(dims[1], L[1], channels[7], kernel_size, patch_size, int(dims[1]*4)))
        self.mvit.append(MobileViTBlock_v2(dims[2], L[2], channels[9], kernel_size, patch_size, int(dims[2]*4)))
...
def mobilevit_v2_s_skip_revise(img_size=(256,256),input_channels=3,mode=None,num_classes=2):
    dims = [144, 192, 240]
...
if __name__ == '__main__':
    ...
    vit = mobilevit_v2_s_skip_revise(input_channels=4,num_classes=2)
    out = vit(img)
    print(out.shape)
