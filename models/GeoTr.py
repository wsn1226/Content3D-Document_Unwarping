...
class TransDecoder(nn.Module):
    ...
    def forward(self, imgf, query_embed):
        pos = self.position_embedding(torch.ones(imgf.shape[0], imgf.shape[2], imgf.shape[3]).to(torch.uint8).cuda())  # torch.Size([1, 128, 36, 36])
...
class GeoTr(nn.Module):
    def __init__(self,num_attn_layers,input_dim=3):
        super(GeoTr, self).__init__()
        self.num_attn_layers = num_attn_layers

        self.hidden_dim = hdim = 256

        self.fnet = BasicEncoder(input_dim=input_dim, output_dim=hdim, norm_fn='instance')
        ...
        self.query_embed = nn.Embedding(256, self.hidden_dim)
    ...
    def forward(self, image1):
        ...
        return ((bm_up/128)-0.5)*2

def count_parameters(model):
    return sum(p.numel() for p in model.parameters() if p.requires_grad)

if __name__ == '__main__':
    img = torch.randn(5, 4, 128, 128).cuda()
    model=GeoTr(6,input_dim=4).cuda()
    result=model(img)
