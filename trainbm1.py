...
def train(args):
    ...
    # Setup Dataloader
    data_loader = get_loader('doc3dbm_withrecon')
    ...
    trainloader = data.DataLoader(t_loader, batch_size=args.batch_size, num_workers=8, shuffle=True,pin_memory=True)
    valloader = data.DataLoader(v_loader, batch_size=args.batch_size, num_workers=8, pin_memory=True)

    # Setup Model
    model = get_model(args.arch, n_classes,in_channels=3, img_size=(args.img_rows, args.img_cols))
    ...
    experiment_name='bm_1' #network_activation(t=[-1,1])_dataset_lossparams_augmentations_trainstart
    ...
    for epoch in range(epoch_start,args.n_epoch):
        ...
        for i, (images, labels) in enumerate(trainloader):
            ...
            rloss = reconst_loss(images[:,3:,:,:],target_nhwc,labels)
            #rloss,ssim,uworg,uwpred = reconst_loss(images[:,:-1,:,:],target_nhwc,labels)
            loss=10*l1loss+0.5*rloss  #+ (0.3*ssim) The L_T loss for the textured mapping network
            # loss=l1loss  
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', nargs='?', type=str, default='./checkpoints-bm1/',    
...
#CUDA_VISIBLE_DEVICES=1 python trainbm1.py --arch mobilevit_sandfcn_skip --data_path /disk2/sinan/doc3d/ --img_rows 128 --img_cols 128 --n_epoch 300 --batch_size 200 --l_rate 0.00005 --tboard --resume /home/sinan/DewarpNet-master/checkpoints-bm1/mobilevit_sandfcn_skip_164_0.00032531899050809444_0.0001209049398564873_bm_1_best_model.pkl