...
def train(args):
    ...
    # Setup Dataloader
    data_loader = get_loader('doc3dabs_depthfromalb')
    ...
    for epoch in range(epoch_start,args.n_epoch):
        ...
        for i, (images, labels) in enumerate(trainloader):
            ...
            labels_msk=(labels[:,0,:,:]>=0).to(torch.int8)
            ...
            #d0_msk, d1_msk, d2_msk, d3_msk, d4_msk, d5_msk, d6_msk=(torch.sign(d0))) , (torch.sign(d1))) , (torch.sign(d2))) , (torch.sign(d3))) , (torch.sign(d4))) , (torch.sign(d5))) , (torch.sign(d6)))
            #print(d0_msk)
            #print(d0[msk_index[:,0],msk_index[:,1],msk_index[:,2]].shape)
            ...
            del d0, d1, d2, d3, d4, d5, d6,only_d0, only_d1, only_d2, only_d3, only_d4, only_d5, only_d6,loss2_depth, loss_depth, loss  
            if (i+1) % 50==0:
        ...
        for i_val, (images_val, labels_val) in tqdm(enumerate(valloader)): #use progress bar
            with torch.no_grad():
                ...
                labels_msk=(labels_val[:,0,:,:]>=0).to(torch.int8)
                msk_index=(labels_msk).nonzero()

                only_d1=pred_val[msk_index[:,0],:,msk_index[:,1],msk_index[:,2]]
                only_d_lbl=labels_val[msk_index[:,0],:,msk_index[:,1],msk_index[:,2]]

                loss_depth=l1_loss(only_d1,only_d_lbl)
                val_loss+=float(loss_depth)
                #val_gloss+=float(g_loss)
                del d1,d2,d3,d4,d5,d6,d7,only_d_lbl,only_d1,labels_msk
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', nargs='?', type=str, default='./checkpoints-u2net_abs_depth_fromalb/',    
...
# CUDA_VISIBLE_DEVICES=1,0,2 python trainu2net_depth_fromalb.py --arch u2net_depth --data_path /disk2/sinan/doc3d/ --batch_size 72 --resume /home/sinan/DewarpNet-master/checkpoints-u2net_abs_depth_fromalb/u2net_depth_88_0.021127871436484164_0.02859664461809954_depth_best_model.pkl