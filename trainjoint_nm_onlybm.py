...
def train(args):
    ...
    #Log file:
    experiment_name='nm_joint_onlybm'
    ...
    for epoch in range(epoch_start,args.n_epoch):
        ...
        for i, (images, norm, bm) in enumerate(trainloader):
            ...
            pred_nm=model_nm(extracted_img)[0]
            masked_nm=pred_nm*pred_msk
            norm=masked_nm.pow(2).sum(dim=1).sqrt()
            #print(torch.max(norm))
            msk=norm.nonzero()
            norm=norm.unsqueeze(1)
            masked_nm[msk[:,0],:,msk[:,1],msk[:,2]] = masked_nm[msk[:,0],:,msk[:,1],msk[:,2]]/norm[msk[:,0],:,msk[:,1],msk[:,2]]

            masked_nm=masked_nm.detach()
            #cv2.imwrite("testnm.png",masked_nm[0,:3,:,:].permute(1,2,0).detach().cpu().numpy()*255)
            bm_input=F.interpolate(masked_nm,(args.bm_img_rows,args.bm_img_cols),mode='bilinear',align_corners=True)

            bm_input=Variable(bm_input)
            pred_bm=model_bm(bm_input)
            #-------------------BM--------------------------
            ...
            del norm, masked_nm,images, bm, pred_bm,predbm_nhwc,pred_msk,loss,loss_bm#,loss_depth,loss2_depth
            if (i+1)%50==0:
        ...
        for i_val, (images_val, nm_val, bm_val) in tqdm(enumerate(valloader)): #use progress bar
            with torch.no_grad():
                ...
                del predbm_nhwc,pred_bm,pred_msk,norm
                if (i_val+1) % 50==0:
                    torch.cuda.empty_cache()
...
#CUDA_VISIBLE_DEVICES=1,0,2 python trainjoint_nm_onlybm.py --data_path /disk2/sinan/doc3d/ --tboard --resume_boundary /home/sinan/DewarpNet-master/checkpoints-u2net-nopre/u2net_132_0.010844790506031037_0.04020660579093357_boundary_best_model.pkl --resume_nm /home/sinan/DewarpNet-master/checkpoints-nm_U2NET/u2net_nm_119_0.005878953804735574_0.007961844027149506_doc3dnm_u2net_best_model.pkl --resume_bm /home/sinan/DewarpNet-master/checkpoints-nm/mobilevit_sandfcn_skip_90_8.652182688820176e-05_0.00011821513049622165_rgb_nm_best_model.pkl